...
class ScenarioRunner(object):
    ...
    def _prepare_ego_vehicles(self, ego_vehicles, spawn_point):
        """
        Spawn or update the ego vehicles
        """

        # generate xml file with random spawn points
        
        #spawn_points = self.world.get_map().get_spawn_points()
        #spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
        #generateRandomSpawnPointsXml(spawn_points, self)
        # ==============================================================================
    ...
    def _load_and_run_scenario(self, config):
        ...
        spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
        copyConfig = config    
        copyConfig.ego_vehicles[0].transform = spawn_point
        copyConfig.trigger_points[0] = spawn_point

        try:
            self._prepare_ego_vehicles(config.ego_vehicles, spawn_point)
...
def generateRandomSpawnPointsXml(spawn_points, self):
    ...
    file = open("FollowLeadingVehicle.xml", "w")
    #file.write("%s" %(point)+ "\n")
    
    with tag('scenarios'):
        for point in spawn_points:
            with tag('scenario', name="FollowLeadingVehicle_1", type="FollowLeadingVehicle", town="Town01"):
                with tag('ego_vehicle' ,x=point.location.x, y=point.location.y, z=point.location.z, yaw=point.rotation.yaw, model="vehicle.tesla.model3"):
                    text('')
